...
| **COMM02 공통 컴포넌트** | `wireframe/COMM02_공통컴포넌트.html` / `docs/기획/screens/COMM02_공통컴포넌트.md` | 버튼, 인풋, 태그/뱃지, 팝업/오버레이, 카드, 네비게이션, 검색 |

- 새 화면에서 버튼, 인풋, 카드 등 공통 컴포넌트를 사용할 때 COMM02에 정의된 스펙(크기, 색상, 상태)을 따를 것
- 컬러, 폰트 크기, 간격 값은 COMM01 디자인 토큰에 정의된 값을 사용할 것
- 새로운 공통 패턴이 발견되면 COMM02에 먼저 추가한 후 화면에 적용할 것
