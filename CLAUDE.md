...
### 디자인 토큰/컴포넌트 고정 지침 (필수)

- 작업 시작 전 `wireframe/COMM01_디자인토큰.html`과 `wireframe/COMM02_공통컴포넌트.html`을 먼저 확인하고 그 스펙 기준으로 작업한다.
- `COMM01_디자인토큰.html`의 `8. 상태 아이콘`은 `wireframe/HOME01_디자인시안.html` 아이콘 톤/스타일과 동일하게 유지한다.
- 디자인 토큰(컬러/타이포/간격/라운딩/모션 등)을 변경하면 `wireframe/COMM02_공통컴포넌트.html`을 반드시 같은 변경으로 즉시 업데이트한다.
...
### 동행 매칭 알림
- 동행글 등록 시 같은 이벤트 찜한 유저에게 알림
- 이벤트 D-3 리마인더: 해당 이벤트 동행글이 있으면 알림
- Phase 2: 선호 장르/지역 기반 정밀 매칭
...
- 화면 기획서: AUTH01~02, EXCH01~03, EXCH05, GRPB01~04, COMM01~02, SRCH01, USER01~03, CHAT01~02, WANT01~02, EVNT01~03, BANR01, NOTI01, STAT01
- 와이어프레임: HOME01, AUTH01~02, EXCH01~03, EXCH05, GRPB01~04, COMM01~02, SRCH01, USER01~03, CHAT01~02, WANT01~02, EVNT01~03, BANR01, NOTI01, STAT01
- IA 사이트맵 v1.7, 유저 플로우 v1.7
