...
- 각 제안 카드:
  ...
  - 액션 버튼 (대기중일 때만): [제안 취소]
  - 수락됨일 때: [채팅으로 이동] 버튼
  - 거절됨/만료일 때 + 상대 교환글이 `active` 상태: [다시 제안하기] Blue 아웃라인 버튼
    - 탭 시 EXCH04(교환 제안)로 이동, 이전 제안 데이터 프리필
    - 상대 교환글이 `closed`/`completed`/`hidden`이면 버튼 숨김 + "교환글이 마감되었습니다" 안내 텍스트
...
| items[].target_exchange_status | string | 상대 교환글 현재 상태 (`active`/`closed`/`completed`/`hidden`) |
| items[].can_repropose | boolean | 재제안 가능 여부 (거절/만료 + 상대글 active + 본인 pending < 3) |
...
| 4-1 | 교환글 마감 시 | 마감된 글의 남은 `pending` 제안 일괄 `expired` 처리 + 제안자에게 "교환글이 마감되었습니다" 알림 |
...
| 10 | 재제안 가능 조건 | 보낸 제안이 `rejected`/`expired` + 상대 교환글 `active` + 해당 글에 본인 `pending` 제안 3건 미만일 때 [다시 제안하기] 표시 |
| 11 | 재제안 데이터 프리필 | [다시 제안하기] 탭 시 EXCH04에 이전 제안 데이터(제안 굿즈, 수량, 차액 등) 프리필 전달 |
| 12 | 마감/비활성 교환글 안내 | 보낸 제안의 상대 교환글이 `closed`/`completed`/`hidden`이면 [다시 제안하기] 숨김 + "교환글이 마감되었습니다" 회색 안내 텍스트 표시 |
...
| 10 | [다시 제안하기] 탭 | EXCH04로 이동 (이전 제안 데이터 프리필) | 거절됨/만료 + 상대글 active |
| 11 | 마감된 교환글 안내 표시 | "교환글이 마감되었습니다" 회색 텍스트, 재제안 불가 | 상대글 closed/completed/hidden |
...
| 날짜 | 버전 | 내용 | 작성자 |
|------|------|------|--------|
| 2026-03-11 | 0.7 | 보낸 제안 재제안(다시 제안하기) UX 추가: CTA, 프리필 로직, 마감 교환글 안내 | - |
| 2026-03-11 | 0.6 | 교환글 마감 시 pending 제안 일괄 만료 처리 규칙 추가 | - |
